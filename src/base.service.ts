...
export class BaseService {
  ...
  async sendCommand(cmd: string, data: object, meta: any, includeMeta?: boolean): Promise<object> {
  ...
  async sendQuery(query: string, data: any, meta: any, includeMeta?: boolean): Promise<any> {
    return this.sendFqq(`queries.${this.domain}.${query}`, data, meta, includeMeta);
  }

  async sendFqq(fqq: string, data: any, meta: any, includeMeta?: boolean): Promise<any> {
    const cid = meta?.correlationId || uuid4();
